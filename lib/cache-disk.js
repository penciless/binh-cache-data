...
function CacheDisk(init_path) {
    ...
    var removals = 0, forgetting = false;
    ...
    cache.forget = function(amount) {
        ...
        openDirectory(data_directory, { encoding: 'utf8', bufferSize: 32 }).then(function(dir) {
            ...
            removals = Math.max(amount, removals);
    
            if (forgetting || removals <= 0 || invalidLoop()) return;
            
            forgetting = true;
    
            function invalidLoop() {
                if (removals > metadata.size) {
                    if (stopRef instanceof Function) stopRef();
                    cache.empty().then(function() {
                        console.log('empty', removals, metadata.size);
                        removals = 0;
                        forgetting = false;
                    }).now();
                    return true;
            ...
            console.log('forget', removals, metadata.size);
