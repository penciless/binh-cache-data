...
function CacheData() {
    ...
    var cache = this, source, edges = [], logConsole = function(){};
    ...
    this.delete = function(id, include_source) {
        var caches = include_source === true ? edges.concat(source ? [source] : []) : edges;
    ...
    this.forget = function(input) {
        if (input instanceof Function) input(edges.slice(0));
            ...
            try {
                ...
                logConsole(`[CacheData] Error forgetting data in cache: ${error}`);
            }
        });
    };
