...
function CacheData() {
    ...
    function getRecursively(id, index, finalize) {
        ...
        try {
            ...
            console.log(`[CacheData] Error getting data from cache: ${error}`);
            getNextCache(id, index, finalize);
        }
    }
    ...
    function saveToEdges(id, data) {
        edges.forEach(function(edge) {
            saveToCache(edge, id, data);
        });
    }

    function saveToSource(id, data) {
        saveToCache(source, id, data);
    }

    function saveToCache(cache, id, data) {
        try {
            cache.save(id, data);
        }
        catch (error) {
            console.log(`[CacheData] Error saving data to cache: ${error}`);
        }
    }
